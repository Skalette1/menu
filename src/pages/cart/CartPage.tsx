...
export const CartPage: React.FC = () => {
  const items = useSelector((state: RootState) => state.cart.items as import("../../shared/model/cartSlice").CartItem[]);
  const dispatch = useDispatch();
    const total = items.reduce((sum: number, item: import("../../shared/model/cartSlice").CartItem) => sum + item.price * item.count, 0);
...
<div className={styles.cartContainer}>
  <div className={styles.tittleContainer}>
    <Link to="/" className={styles.arrowToHome}>←</Link>
              ...
              {item.count} * {item.price}₽ = {item.count * item.price}₽
                ...
                <div className={styles.counter}>
                  <button
                    ...
                    onClick={() => dispatch(setCount({ id: item.id, count: Math.max(1, item.count - 1) }))}
                  >
                    -
                  </button>
                  ...
                  <button
                    className={styles.counterBtn}
                    onClick={() => dispatch(setCount({ id: item.id, count: item.count + 1 }))}
                  >
                    +
                  </button>
                </div>
  ...
  <div className={styles.totalWrapper}>
    ...
    <div className={styles.total}><span>Итого:</span> {total}₽</div>
  </div>
</div>
